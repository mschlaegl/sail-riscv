...
void run_model(CLIOptions &opts, ModelImpl &model, uint64_t, const elf_info &elf_info, run_info &run_info) {
  ...
  if (opts.has_until_pc) {
    until_pc = std::make_shared<until_pc_callbacks>(opts.until_pc);
    model.register_callback(until_pc);
  }
  ...
  do {
    run_sail(model, opts, loop_detector, until_pc, elf_info, run_info);
