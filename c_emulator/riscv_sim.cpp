...
#include "riscv_callbacks_until_pc.h"
...
void write_memory_dump(const MemoryDumpRegion &region) {
  std::ostringstream file_os;
  file_os << "mem.0x" << std::hex << region.base << ".bin";
...
void write_memory_dumps(const std::vector<MemoryDumpRegion> &regions) {
  for (const auto &region : regions) {
    write_memory_dump(region);
  }
}
...
void finish(ModelImpl &model, const CLIOptions &opts, const elf_info &elf_info, run_info &run_info) {
  ...
  if (opts.do_dump_memory) {
    write_memory_dumps(model.main_memory_regions());
  }
...
void run_sail(
  ...
  while (!model.htif_done() && !(until_pc && until_pc->stop_requested()) &&
...
InitResult preinit_args(CLIOptions &opts, std::string &config_json_string) {
  ...
  if (opts.do_dump_memory) {
    fprintf(stderr, "will dump main memory on completion.\n");
  }
